...
- per-card Spending Power, limiting-date account/receivable reconciliation, payment-date funding snapshots, and future funding-account lows;
...
- occurrence-specific recurring receivables, including partial and late receipts, a later-installment prepayment, cash received before the paired expense, static-plus-recurring anchors, no double accrual, transactional edit/delete reversal, cross-user rollback, and rejection of a future “received” date before mutation;
...
- schema-22 debt metadata, schema-23 loan-payment treatment, schema-24 revolving-account lifecycle, schema-25 amortization structure/contractual-balloon persistence, schema-26 bill-relative receivable timing, and schema-27 durable receipt-occurrence identity/target metadata with legacy-safe nullable defaults and portable-backup round trips.

The current intended public tree passed 463 synthetic tests across 43 files for the daily-driver and publication/privacy behaviors above. Seventeen additional disposable-Git and child-process privacy cases were discovered but skipped because the restricted environment blocks Node child-process creation; they run automatically in a normal development or CI environment. The packaged Electron application from the `dff8701` product baseline also passed its isolated smoke check. The same process restriction blocked the full Playwright Electron journey, so no complete `pnpm verify`, end-to-end UI, responsive-layout, or theme-accessibility pass is claimed for the current 1.1 candidate; those checks must run in an unrestricted release environment.

Standalone `pnpm test:e2e` runs package the current source before launching Electron, use a five-minute suite and native-rebuild hook budget, and scope mutation-result status assertions by their message. This prevents a slow loading skeleton from colliding with a success announcement and prevents a journey-only run from silently exercising stale `.vite` output.
...
| Account minimum or preferred buffer                                                                                      | Account breach/margin, effective global minimum/preferred value, Overview protected floor, Spending Power, and Funding Actions                                                                                                                            |
...
| Card terms, statement/payment state, issuer snapshot, carrying amount, lifecycle, current spending, or purchase scenario | Statement due, total/current/carrying debt, open-cycle activity, available credit, owning cycle, closure eligibility, projected obligation, actual payment date, Spending Power, advisor rank, Net Worth, and transfer requirement                        |
| Receivable amount, accrual, receipt, or selected occurrence                                                              | Owed asset, planned/actual cash, exact installment remainder, total-position identity, and net-worth agreement                                                                                                                                            |
