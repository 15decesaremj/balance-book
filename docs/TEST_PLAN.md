...
- release-only recurring receivables keeping future installments out of current Money Owed, accruing them only on their selected occurrence dates, creating no planned bank cash, and applying full or partial releases only to the linked occurrence and selected destination account with audit lineage;
...
The current intended public tree passed 561 synthetic tests across 51 files. The disposable-Git and child-process privacy cases, production package, and full Playwright journey passed in the same unrestricted local pipeline, including end-to-end input/output propagation, both-theme accessibility checks, receivable release, card payment overrides, carry/credit, charts, current-cycle account-risk windows, bounded release metadata, and a 120-state visual invariant matrix across every authenticated route at eight widths from 1440px through 320px. The exact unsigned 1.1.2 candidate also passed owner-profile upgrade validation: installed/package hashes matched, the database remained unchanged before first launch, schema 30 applied on launch, and SQLite integrity remained `ok`. A fresh disposable Windows-profile lifecycle remains a separate release gate because the hardened installed executable's disabled Node-inspection fuse intentionally prevents Playwright attachment.
